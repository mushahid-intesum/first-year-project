...
int scorefunc()
{
	...
	while(!quit)
	{
		...
		// SDL_SetRenderDrawColor(gRenderer,120,120,120,120);
		// SDL_RenderClear(gRenderer);
		// //gScore.render(0,0);
		// gScore.LoadFromRenderedText(s,textColor);
		// gScore.render( SW/4 , SH/4);
		// SDL_RenderPresent(gRenderer);
	}
...
int show_hs()
{
	...
	char s[10000];
	sprintf(s,"HIGH SCORE : %d",hiscore);
...
bool loadMediaStart()
{
	...
	bool success = true;
	gFont = TTF_OpenFont( "Rubik-Regular.ttf", 50);
	//Load texture
